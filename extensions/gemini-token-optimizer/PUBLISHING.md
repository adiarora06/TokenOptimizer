...
- Uses the deployed Token Optimizer endpoint by default.
...
- Prompt text is sent to the configured Token Optimizer backend only when the user clicks Optimize.
...
- The extension stores local settings and the latest raw prompt only to prevent recursive wrapper output.
